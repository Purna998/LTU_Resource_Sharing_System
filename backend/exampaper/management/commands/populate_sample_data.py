...
class Command(BaseCommand):
    help = 'Aggressively populates database with 4 departments, 8 semesters each, and sample subjects/resources.'

    def handle(self, *args, **kwargs):
        self.stdout.write('Wiping old data...')
        Department.objects.all().delete()

        self.stdout.write('Populating Platform data...')

        departments_data = [
            {'name': 'Computer Science & Engineering', 'short': 'CS & E', 'icon': 'Monitor'},
            {'name': 'Information Technology', 'short': 'IT', 'icon': 'Laptop'},
            {'name': 'Electrical Engineering', 'short': 'EE', 'icon': 'Cpu'},
            {'name': 'Civil Engineering', 'short': 'Civil', 'icon': 'HardHat'},
        ]
        ...
        for dept_info in departments_data:
            dept = Department.objects.create(
                ...
                description=f"Welcome to the {dept_info['name']} department. Find all your syllabuses, notes, and past papers here.",
            ...
            for i, sem_name in enumerate(semester_names, start=1):
                ...
                for sub_name, code in subs:
                    ...
                    res_syl = SubjectResource.objects.create(
                        subject=subject, title=f'{sub_name} Syllabus (Official)', resource_type='syllabus', contributor='System Admin'
                    )
                    ...
                    for year in [2078, 2079, 2080]:
                        res_oq = SubjectResource.objects.create(
                            subject=subject, title=f'{year} Regular Exam Paper', resource_type='old_question', year=year, contributor='System Admin'
                        )
        ...
        # Global Notices
        Notice.objects.create(title='First Semester Result Published', content='The results for all First Semester students have been published. Check the portal.', department=None)
        Notice.objects.create(title='Academic Calendar', content='The official academic calendar for the upcoming session has been finalized.', department=None)

        self.stdout.write(self.style.SUCCESS('Successfully populated data with enormous depth!'))
